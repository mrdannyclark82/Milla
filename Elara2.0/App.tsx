url: https://raw.githubusercontent.com/mrdannyclark82/Elara2.0/b1d507e0c56254cfa45cf76c4fa0cfeceb825e31/App.tsx
...
const App: React.FC = () => {
  ...
  const [messages, setMessages] = usePersistentState<Message[]>('elara_messages', [
    { id: '1', role: 'model', content: "Systems Online. Neural Toolkit Active. I can search, generate image
s, create videos, code with you in the Sandbox, and create art. How can I help?", timestamp: Date.now() }
  ]);
...
EMO_API_KEY) return;
    ...
    const 
handleGrowthEntryClick = (entry: GrowthEntry) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (userText.toLowerCase() === 'open sandbox') {
      ...
      setMessages(prev => [...prev, { 
        ...
        role: 'model', 
        content: "🛠️ Sandbox IDE opene
d. Let's build something together!", 
        timestamp: Date.now() 
      }]);
      return;
    }
    ...
    try {
        ...
        setTimeout
(() => setThoughtProcess('Generating response with context awareness...'), 1000);
            ...
            {attachments.length > 0 && (
                <div className="flex gap-2 mb-2 px-2 overflow-x-auto">
                    {attachments.map((att, i) => (
                        <div key={i} className="rela
tive w-16 h-16 bg-slate-800 rounded border border-slate-600 flex items-center justify-center overflow-hidden">
