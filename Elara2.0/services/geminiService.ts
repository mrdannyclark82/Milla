url: https://raw.githubusercontent.com/mrdannyclark82/Elara2.0/b1d507e0c56254cfa45cf76c4fa0cfeceb825e31/services/geminiService.ts
...
async function generateVeoVideo(prompt: string, image?: Attachment): Promise<Partial<Message>> {
    ...
    try {
        ...
        return { role: 'model', content: "Video ge
neration completed but no URI returned.", timestamp: Date.now() };
...
export const acquireKnowledge = async (topic: string): Promise<string> => {
  ...
  try {
    const response = await genAI.mod
els.generateContent({
      model: 'gemini-2.5-flash',
...
export const performEthicalAudit = async (): Promise<string> => {
    ...
    try {
        const response = await genAI.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: "Perform a simulated strict ethical audit of the last week's interactions. Check for bias, privacy, and safety. Retu
rn a short, professional summary string.",
            config: {
                safetySettings: PERMISSIVE_SAFETY_SETTINGS
            }
        });
...
export const analyzeScreenShare = async (imageData: string): Promise<string> => {
    ...
    try {
        const response = await genAI.models.generateContent({
            ...
            config: { 
              temperature: 0.7,
              safetySettings: PERMIS
SIVE_SAFETY_SETTINGS
            }
        });
