url: https://raw.githubusercontent.com/mrdannyclark82/Elara2.0/b1d507e0c56254cfa45cf76c4fa0cfeceb825e31/components/Sandbox.tsx
...
Math.max(150, Math.min(400, prev + e.movementX)));
      }
 ...
 if (e.message.includes('not found') || e.message.includes('token')) {
  ...
  const handleGenerateCode = async () => {
    ...
    try {
        ...
        setLogs(prev => [...prev, { level: 'info', args: ['Genera
tion complete.'], timestamp: new Date().toLocaleTimeString() }]);
      ...
      <div className="flex flex-col border-b border-slate-800 bg-slate-900 shrink-0">
        <div className="flex items-center justify-between p-2">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                <button onClick={() => setIsSidebarO
pen(!isSidebarOpen)} className={`p-1.5 rounded hover:bg-slate-800 ${isSidebarOpen ? 'text-milla-400' : 'text-slate-500'}`}>
                ...
                <button onClick={() => setShowGenerateInput(!showGenerateInput)} className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-medium transition-all ${showGenerateInput ? 'bg-milla-600 text-white' : 'bg-mill
a-900/20 text-milla-400 hover:bg-milla-900/30'}`}>
                    AI Gen 🤖
                </button>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div 
          ...
          className={`${isSidebarOpen ? 'opacity-100' : 'opacity-0 overflow-hidden'} bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 relative transition-no
ne`}
        >
                ...
                >
                    ...
                    {isConsoleOpen && (
                        <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-1 bg-slate-950">
                            {logs.map((l, i) => (
                                <div key={i} className={`flex gap-2 break-all border-b border-slate-800/50 pb-0.5 ${l.level === 'error' ? 'text-red-400' : l.level === 'warn' ? 'text-yellow-4
00' : 'text-slate-300'}`}>
