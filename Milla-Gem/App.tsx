url: https://raw.githubusercontent.com/mrdannyclark82/Milla-Gem/00b4fc6539267fe8d29b65b188e1ad9d094cbacc/App.tsx
...
function App() {
  ...
  const [isGoogleConnected, setIsGoogleConnected] = useState(() => l
ocalStorage.getItem('milla-google-connected') === 'true');
  ...
  // Live Conversation
  const handleTranscription = useC
allback((isFinal: boolean, text: string, sender: 'user' | 'ai') => {
      setMessages(prev => {
  ...
  const fetchAndSetSuggestedAction = async () => {
     // Create a simplified history for the suggestion prompt
      const history = 
messages.slice(-6).map((msg): { role: string, parts: Part[] } => ({
            ...
            if (m.toolCalls?.some(tc => tc.id === toolCall.id))
 {
  ...
  const handleCancelTool = (toolCallId: 
